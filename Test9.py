...
class menu1(object):
    def __init__(self):
        self.img = pygame.image.load('titlescreen.png')
        self.img.set_colorkey(WHITE)
        self.button_number = button('Start Numbers', 660-5, 340-45, 200, 80, 26)
        self.button_tutor = button('Tutorial', 660-5, 500-45, 200, 80, 30)
        self.button_quit = button('Quit', 660-5, 580-45, 200, 80, 30)
        self.button_kana = button('Start Kana', 660-5, 420-45, 200, 80, 28)
...
def runGame(choice, kind, is_tutorial):
    ...
    ##Start first stroke hitbutton
    Backgrd.draw()
    firstbutton = HitButton(characters[choice-1].return_points()[0])
    while firstbutton.radius>1:
